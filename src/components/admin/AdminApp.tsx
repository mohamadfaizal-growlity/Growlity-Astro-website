...
const Sidebar = ({ schemas, siteLogo, onLogout, onHoverChange }: { schemas: any[], siteLogo?: string, onLogout?: () => void, onHoverChange?: (hovered: boolean) => void }) => {
  ...
  return (
    <div 
      className="w-[72px] hover:w-64 group bg-white/80 backdrop-blur-md border-r border-slate-200/50 text-slate-600 h-screen flex flex-col shadow-[4px_0_24px_rgba(0,0,0,0.02)] fixed top-0 left-0 z-50 transition-all duration-300 overflow-hidden hover:shadow-2xl"
      onMouseEnter={() => onHoverChange && onHoverChange(true)}
      onMouseLeave={() => onHoverChange && onHoverChange(false)}
    >
      <div className="h-16 flex items-center px-4 border-b border-slate-200/50 flex-shrink-0">
        <div className="w-10 flex-shrink-0 flex items-center justify-center">
          <img src="/G-Icon.png" alt="Growlity Icon" className="h-8 w-8 object-contain group-hover:hidden" />
          {siteLogo ? (
            <img src={siteLogo} alt="Site Logo" className="h-8 w-auto max-w-[120px] object-contain hidden group-hover:block" />
          ) : (
            <img src="/growlity-logo.png" alt="Growlity" className="h-8 w-auto max-w-[120px] object-contain hidden group-hover:block" />
          )}
        </div>
        {!siteLogo && (
...
const AdminLayout = ({ schemas, siteLogo, setIsAuthenticated }: any) => {
  ...
  return (
    <div className="flex min-h-screen bg-[#F8FAFC]">
      ...
      <main className={`flex-1 overflow-x-hidden transition-all duration-300 ${!isEditorRoute ? (isSidebarHovered ? 'ml-64' : 'ml-[72px]') : ''}`}>
        <Routes>
